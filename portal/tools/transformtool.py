...
class _MoveOperation(BaseTool):
    ...
    def __init__(self, canvas):
        ...
        self.before_image: QImage | None = None
...
class _ScaleOperation(BaseTool):
    ...
    def _current_edge_rect_doc(self) -> QRectF | None:
        ...
        if (
            self._scaled_edge_rect_doc is not None
            and not self._scaled_edge_rect_doc.isEmpty()
        ):
            return QRectF(self._scaled_edge_rect_doc)

        self._ensure_base_rect()
        if (
            self._base_edge_rect_doc is not None
            and not self._base_edge_rect_doc.isEmpty()
        ):
...
class TransformTool(BaseTool):
    ...
    def __init__(
        ...
        *,
        ...
        self._move_tool = move_tool or _MoveOperation(canvas)
        self._rotate_tool = rotate_tool or _RotateOperation(canvas)
    ...
    def mouseMoveEvent(self, event, doc_pos):
        ...
        if self._active_operation == "rotate":
            ...
            buttons_attr = getattr(event, "buttons", None)
            if buttons_attr is None:
                buttons_state = getattr(Qt.LeftButton, "value", 0)
            elif callable(buttons_attr):
                buttons_state = buttons_attr()
            else:
                buttons_state = buttons_attr
            buttons_value = getattr(buttons_state, "value", buttons_state)
            try:
                buttons_value = int(buttons_value)
            except (TypeError, ValueError):
                buttons_value = 0
            left_button_value = getattr(Qt.LeftButton, "value", 0)
            dragging_move = bool(
                self._active_operation == "move"
                and buttons_value & left_button_value
            )
            self._set_dragging_transform(dragging_move)
    ...
    def _handle_scale_finished(self) -> None:
        consume = getattr(self._scale_tool, "consume_did_apply_scale", None)
        did_scale = bool(consume()) if callable(consume) else False
        self._refresh_scale_handles()
        if did_scale:
            reset_pivot = getattr(self._rotate_tool, "reset_pivot_to_default", None)
    ...
    def _handle_move_finished(self, delta: QPoint) -> None:
        ...
        offset_pivot = getattr(self._rotate_tool, "offset_pivot", None)
        if pivot_manual:
            if not remaining_delta.isNull() and callable(offset_pivot):
                offset_pivot(remaining_delta)
        else:
            selection_shape = getattr(self.canvas, "selection_shape", None)
            if (
                selection_shape is not None
                and not remaining_delta.isNull()
                and callable(offset_pivot)
            ):
                offset_pivot(remaining_delta)
            self._refresh_pivot()
        self._refresh_scale_handles()
    ...
    def _refresh_scale_handles(self) -> None:
        refresh = getattr(self._scale_tool, "refresh_handles_from_document", None)
        if callable(refresh):
            refresh()
        self._update_rotation_overlay_geometry()
    ...
    def _refresh_pivot(self) -> None:
        refresh = getattr(self._rotate_tool, "refresh_pivot_from_document", None)
        if callable(refresh):
            refresh()
        self._update_rotation_overlay_geometry()
    ...
    def _update_rotation_overlay_geometry(self) -> None:
        get_overlay = getattr(self._scale_tool, "get_overlay_geometry", None)
        overlay = get_overlay() if callable(get_overlay) else None
        if overlay is None:
            rect_canvas = None
            handle_rects: dict[str, object] = {}
        else:
            rect_canvas, handle_rects = overlay
        set_geometry = getattr(self._rotate_tool, "set_overlay_geometry", None)
