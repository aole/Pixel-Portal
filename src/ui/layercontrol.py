...
LayerRenameEvent,       EVT_LAYER_RENAME_EVENT = wx.lib.newevent.NewEvent()
...
class LayerPanel(wx.Panel):
    def __init__(self, parent=None):
        ...
        self.alphabg = wx.Bitmap("alphabg.png").GetSubBitmap(wx.Rect(
            0, 0, 300, 300)).ConvertToImage().AdjustChannels(1, 1, 1, .3).ConvertToBitmap()
        self.bmVisible = wx.Bitmap("icons/visible.png")
        self.bmNotVisible = wx.Bitmap("icons/NA.png")

        self.textctrl = wx.TextCtrl(self, value="layer", pos=(
            60, 15), size=(100, wx.DefaultCoord), style=wx.TE_PROCESS_ENTER)
    ...
    def OnPaint(self, e):
        ...
        if self.layers:
            ...
            for layer in self.layers:
                ...
                gc.DrawBitmap(self.alphabg, border, y+border,
                              50-border*2, 50-border*2)
                # draw the layer
                gc.DrawBitmap(layer, border, y+border,
                              50-border*2, 50-border*2)
                # print layer name
                if layer == self.layers.Current():
                    gc.SetFont(wx.Font(10, wx.FONTFAMILY_DEFAULT, wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_BOLD),
                               wx.SystemSettings.GetColour(wx.SYS_COLOUR_LISTBOXHIGHLIGHTTEXT))
                else:
                    gc.SetFont(wx.Font(10, wx.FONTFAMILY_DEFAULT, wx.FONTSTYLE_NORMAL,
                               wx.FONTWEIGHT_LIGHT), wx.SystemSettings.GetColour(wx.SYS_COLOUR_LISTBOXTEXT))
                ...
                gc.SetPen(wx.ThePenList.FindOrCreatePen(
                    wx.SystemSettings.GetColour(wx.SYS_COLOUR_BTNSHADOW), border))
                gc.StrokeLine(border/2, y+border/2, w-border/2, y+border/2)
                gc.StrokeLine(border/2, y+border/2, border/2, y+50-border/2)
                gc.SetPen(wx.ThePenList.FindOrCreatePen(
                    wx.SystemSettings.GetColour(wx.SYS_COLOUR_ACTIVEBORDER), border))
                gc.StrokeLine(border/2, y+50-border/2,
                              w-border/2, y+50-border/2)
                gc.StrokeLine(w-border/2, y+border/2,
                              w-border/2, y+50-border/2)
    ...
    def SetTextBox(self, idx=-1, layer=None, x=0, y=0):
        if not layer:
            if self.textctrl and self.textctrl.Shown:
                self.Rename(self.txtBoxLayer, self.textctrl.GetValue())
            self.textctrl.Hide()
            return
    ...
    def Rename(self, layer, name):
        evt = LayerRenameEvent(layer=layer, oldName=layer.name, newName=name)
...
class LayerControl(wx.ScrolledWindow):
    ...
    def OnLeftDown(self, e):
        ...
        if layer:
            if self.panel.IsVisibleIcon(idx, x, y):
                evt = LayerVisibilityEvent(
                    layer=layer, index=idx, position=e.Position)
                wx.PostEvent(self, evt)
            else:
                evt = LayerClickedEvent(
                    layer=layer, index=idx, position=e.Position)
                wx.PostEvent(self, evt)
    ...
    def OnLeftUp(self, e):
        ...
        if self.grabbedLayer and self.panel.highlightSection >= 0:
            evt = LayerDropEvent(layer=self.grabbedLayer,
                                 position=self.panel.highlightSection)
            wx.PostEvent(self, evt)
        self.grabbedLayer = None
        self.panel.highlightSection = -1
        self.panel.Refresh()
