...
from ..layermanager import Layer, LayerManager
...
class AnimationPanel(wx.Panel):
    def __init__(self, parent=None, document=None):
        ...
        self.CURRENT_FRAME_COLOR = wx.Colour(
            *wx.SystemSettings.GetColour(wx.SYS_COLOUR_HIGHLIGHT).Get(False), 50)
    ...
    def DeleteKey(self, frame=None):
        ...
        if not frame in self.document.keys:
            return None
    ...
    def OnPaint(self, e):
        ...
        gc.SetBrush(wx.TheBrushList.FindOrCreateBrush(c))
        gc.SetPen(wx.ThePenList.FindOrCreatePen(wx.SystemSettings.GetColour(
            wx.SYS_COLOUR_ACTIVEBORDER), 2, wx.PENSTYLE_LONG_DASH))
        if self.highlightedSlot[2] > 0:
            x = sab + (self.highlightedSlot[0]-1) * hs
            sh = (h - INFO_BAR_HEIGHT*2) / self.highlightedSlot[2]
            gc.DrawRoundedRectangle(
                x+2, INFO_BAR_HEIGHT + sh * self.highlightedSlot[1] + 2, hs - 4, sh - 4, 5)

        # vertical lines for each frame
        gc.SetBrush(wx.NullBrush)
        gc.SetPen(wx.ThePenList.FindOrCreatePen(
            wx.SystemSettings.GetColour(wx.SYS_COLOUR_ACTIVEBORDER), 1))
        ...
        if pos-mx < 0:
            pos = mx
        gc.DrawRoundedRectangle(pos - NUN_LABEL_WIDTH/2,
                                0, NUN_LABEL_WIDTH, INFO_BAR_HEIGHT, 5)
        gc.SetFont(wx.Font(LABEL_FONT_SIZE, wx.FONTFAMILY_DEFAULT, wx.FONTSTYLE_NORMAL,
                   wx.FONTWEIGHT_LIGHT), wx.SystemSettings.GetColour(wx.SYS_COLOUR_BTNTEXT))
        ...
        gc.DrawRoundedRectangle(pos - NUN_LABEL_WIDTH/2,
                                0, NUN_LABEL_WIDTH, INFO_BAR_HEIGHT, 5)
        gc.SetFont(wx.Font(LABEL_FONT_SIZE, wx.FONTFAMILY_DEFAULT, wx.FONTSTYLE_NORMAL,
                   wx.FONTWEIGHT_LIGHT), wx.SystemSettings.GetColour(wx.SYS_COLOUR_HIGHLIGHT))
        ...
        gc.SetBrush(wx.TheBrushList.FindOrCreateBrush(
            wx.SystemSettings.GetColour(wx.SYS_COLOUR_BTNHIGHLIGHT)))
        gc.DrawRoundedRectangle(sab-10, h-INFO_BAR_HEIGHT,
                                tf*hs+20, INFO_BAR_HEIGHT, 5)
        gc.SetFont(wx.Font(LABEL_FONT_SIZE, wx.FONTFAMILY_DEFAULT, wx.FONTSTYLE_NORMAL,
                   wx.FONTWEIGHT_LIGHT), wx.SystemSettings.GetColour(wx.SYS_COLOUR_BTNTEXT))
        ...
        for f in range(0, lstfrm, gap):
            if f > tf:
                gc.SetFont(wx.Font(LABEL_FONT_SIZE, wx.FONTFAMILY_DEFAULT, wx.FONTSTYLE_NORMAL,
                           wx.FONTWEIGHT_LIGHT), wx.SystemSettings.GetColour(wx.SYS_COLOUR_GRAYTEXT))
    ...
    def SetCurrentFrame(self, frame):
        if self.document.currentFrame != frame:
            ...
            evt = FrameChangedEvent(
                key=key, frame=frame, lastFrame=self.document.currentFrame)
    ...
    def SetDocument(self, document):
        ...
        evt = FrameChangedEvent(
            key=key, frame=self.document.currentFrame, lastFrame=self.document.currentFrame)
        wx.PostEvent(self, evt)
        self.Refresh()
    ...
    def ZoomOnPosition(self, x, y, amt):
        ...
        self.startAnimationBlock -= int((self.horizontalScale*self.document.totalFrames-ps)*(
            (x - self.startAnimationBlock)/(ps)))
        self.Refresh()
...
class AnimationView(wx.Panel):
    ...
    def OnPaint(self, e):
        ...
        w, h = self.GetClientSize()
        if self.bitmap:
            gc.DrawBitmap(self.bitmap, max(0, (w-self.bitmap.width)/2), max(0,
                          (h-self.bitmap.height)/2), self.bitmap.width, self.bitmap.height)
...
class AnimationControl(wx.Window):
    def __init__(self, parent=None, document=None):
        ...
        btn = wx.BitmapButton(
            conpanel, bitmap=wx.Bitmap("icons/keydelete.png"))
        ...
        self.txtFPS.SetFont(wx.Font(14, wx.FONTFAMILY_DEFAULT,
    ...
    def OnFPSChange(self, e):
        num = wx.GetNumberFromUser(
            "Enter FPS", "FPS", "Pixel Portal", self.document.fps, 1, 1000, self)
        if num > 0:
    ...
    def OnLeftDClick(self, e):
        ...
        if gk:
            evt = VisibilityChangedEvent(
                key=gk[0], frame=self.document.currentFrame)
            wx.PostEvent(self, evt)
    ...
    def OnLeftUp(self, e):
        ...
        if self.panel.grabbedKey and not self.panel.document.currentFrame == self.panel.grabbedFrame:
            evt = KeySetEvent(
                key=self.panel.grabbedKey[0], frame=self.panel.document.currentFrame, fromFrame=self.panel.grabbedFrame)
            wx.PostEvent(self, evt)
