...
class Frame(wx.Frame):
    def __init__(self):
        ...
        self.AddMenuItem(
            mselect, "&Invert Selection\tCtrl+Shift+I", self.OnSelectInvert)
    ...
    def CheckAIModels(self):
        ...
        if not os.path.exists(model):
            if not self.DownloadAIModel(model_url, model):
                return False

        if not os.path.exists(lora):
            if not self.DownloadAIModel(lora_url, lora):
                return False

        return True

    def DownloadAIModel(self, url, filename):
        try:
